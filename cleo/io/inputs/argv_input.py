...
class ArgvInput(Input):
        ...
        for i, token in enumerate(self._tokens):
            if token and token[0] == "-":
                ...
                if name not in self._options and not self._definition.has_shortcut(
                    ...
                    name in self._options and self._tokens[i + 1] == self._options[name]
                ) or (
                    self._definition.shortcutToName(name) in self._options
                    and self._tokens[i + 1]
                    == self._options[self._definition.shortcutToName(name)]
                ):
                    is_option = True
    ...
    def _add_short_option(self, shortcut: str, value: Any) -> None:
        if not self._definition.has_shortcut(shortcut):
            raise RuntimeException(f'The "-{shortcut}" does not exist')
    ...
    def _add_long_option(self, name: str, value: Any) -> None:
        if not self._definition.has_option(name):
            raise RuntimeException(f'The "--{name}" does not exist')
        ...
        if value in ["", None] and option.accepts_value() and self._parsed:
            # If the option accepts a value, eitehr required or optional,

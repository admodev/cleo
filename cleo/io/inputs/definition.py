...
class Definition:
    ...
    def has_argument(self, name: Union[str, int]) -> Argument:
        if isinstance(name, int):
            arguments = list(self._arguments.values())
        else:
            arguments = self._arguments
    ...
    def add_option(self, option: Option) -> None:
        ...
        if option.shortcut:
            ...
            if (
                option.shortcut in self._shortcuts
                and option != self._shortcuts[option.shortcut]
            ):
                raise LogicException(
                    f'An option with shortcut "{option.shortcut}" already exists'
                )
        ...
        if option.shortcut:
            # TODO: multiple shortcuts
            self._shortcuts[option.shortcut] = option.name
